...
void TitleMatcherEvaluator::evaluate(RecordMatcherBase* recordMatcher,
    const string& type) {
  ...
  if (fileToWrite.good()) {
    ...
    fileToWrite << "[YES|NO] TAB running time overall (in ms);"
      ...
      "fetch top-k (in ms) TAB groundTruthKey <-> foundedKey TAB query" << endl;
    ...
    for (it = _groundTruth.begin(); it != _groundTruth.end(); it++) {
      ...
      recordMatcher->findBestMatchingRecords(query, &recordScores,
          &runTimesInMs);
      ...
      if (matchedRecord->key.compare(groundTruthKey) == 0 || (groundTruthKey
          == "NO_MATCH" && matchedRecord->key.empty())) {
        fileToWrite << "YES";
        numOfMatches++;
      } else {
        fileToWrite << "NO";
        // TODO: Take into account, that query has no member variable "raw" anymore.
//        matchingFailsFile << "XXX" << '\t' << query.raw << '\t'
//            << groundTruthKey << endl;
//        cout << endl << "Matching failed for: " << groundTruthKey << "(\""
//            << query.raw << "\")" << endl;
...
//      fileToWrite << query.raw;
    ...
    double timeInMs = ((end.tv_sec - start.tv_sec) * 1000000 + end.tv_usec
    ...
    cout << "Number of correct matches: " << numOfMatches << " (" << 100
        * (static_cast<double> (numOfMatches) / numOfQueries) << "%)" << endl;
    cout << "Number of failed matches: " << numOfFails << " (" << 100
        * (static_cast<double> (numOfFails) / static_cast<double> (numOfFails
            + numOfMatches)) << "%)" << endl;
    cout << "Needed Time: Overall: " << timeInMs << "ms, " << "Avg per query: "
        << timeInMs / numOfQueries << "ms" << endl;
    ...
    cerr << "Error: Cannot find file " << (type + FILE_EXTENSION_EVALUATION)
        << endl;
    exit(1);
  }
}
...
// _____________________________________________________________________________
void TitleMatcherEvaluator::readGroundTruth(const std::string& baseName) {
  // Open file, containing the groundtruth (name of file given by baseName)
  FILE *groundTruthFile;
  std::string fileName = baseName;
  fileName.append(".qrels");
  groundTruthFile = fopen(fileName.c_str(), "r");
  char line[MAX_LINE_LENGTH];

  if (groundTruthFile == NULL) {
    // failure on fopen. Probably, there isn't a file with name <baseName>
    perror("fopen failed");
    exit(errno);
  }

  // Read file line by line, line of form: query TAB key
  while (fgets(line, MAX_LINE_LENGTH, groundTruthFile) != NULL) {
    size_t start, pos1, pos2;
    std::string lineAsString = std::string(line);
    // Pair: (query, key)
    std::pair<std::string, std::string> groundTruthElement;

    start = lineAsString.find("\t");
    // Search for the TAB
    pos1 = start + 1;
    pos2 = lineAsString.find("\t", pos1);
    // query = substring of line between the beginning an the pos. of TAB
    groundTruthElement.first = lineAsString.substr(pos1, pos2 - pos1);

    pos1 = pos2 + 1;
    // Search for end of line
    pos2 = lineAsString.find("\n", pos1);
    // query = substring of line between pos. of TAB + 1 and end of line
    groundTruthElement.second = lineAsString.substr(pos1, pos2 - pos1);
