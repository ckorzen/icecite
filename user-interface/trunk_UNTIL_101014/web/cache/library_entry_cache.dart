...
class LibraryEntryCache extends Cache<String, LibraryEntry> {
  ...
  void libraryEntryDeletedHandler(String entryId) {
    if (entryId != null) _uncacheLibraryEntry(getLibraryEntryOrTopic(entryId));
  }
  ...
  LibraryEntry getLibraryEntryOrTopic(String entryId) {
  ...
  /// Returns the topics.
  Iterable<LibraryEntry> getTopics([topicIds]) {
    if (topicIds != null) {
      List<LibraryEntry> res = [];
      topicIds.forEach((topicId) {
        if (topicsCache[topicId] != null) res.add(topicsCache[topicId]);
      });
      return res;
    } 
...
//        LibraryEntry cachedEntry = this[entry.id];
//        if (cachedEntry == null || cachedEntry.rev.compareTo(entry.rev) < 0) {
//          // change entry, if there is no cached entry or if the cached
//          // entry is older than the entry to insert.
          this[entry.id] = entry;
...
//        LibraryEntry cachedTopic = topicsCache[entry.id];
//        if (cachedTopic == null || cachedTopic.rev < entry.rev) {
          // change topic, if there is no cached topic or if the cached
          // topic is older than the topic to insert.
