...
class DbChangeAction {
  /// The type of this change.
  final String _change;
  /// The internal constructor.
  const DbChangeAction._internal(this._change);
  
  /// The type "update".
  static const UPDATED = const DbChangeAction._internal('updated');
  
  /// The type "delete".
  static const DELETED = const DbChangeAction._internal('deleted');
    
  /// Returns a string representation of this change.
  String toString() => _change;
}
...
abstract class PouchDb {
  ...
  Future<Pouchable> put(var pouchable, [options]) {        
    // Create jsObjects.
    JsObject pouchableJs = new JsObject.jsify(pouchable is Map ? pouchable : pouchable.toMap());
  ...
  Future<Pouchable> post(Pouchable pouchable, [options]) {    
    ...
    callback(JsObject error, [JsObject response]) {
      if (error != null) {
        ...
        pouchable['_id'] = map['id'];
        pouchable['_rev'] = map['rev'];
  ...
  /// Retrieves a document, specified by docid.
  Future<List<Map>> getConflictedRevs(String id, List revs, [options]) {           
    if (options == null) options = {};    
    options['open_revs'] = revs;
    JsObject optionsJs = new JsObject.jsify((options != null) ? options : {});
    ...
    callback(JsObject error, [JsObject response]) {
      if (error != null) {
        ...
        _dartify(response).forEach((pouchable) {
          ...
          var rev = pouchable['ok'];
          if (rev['_deleted'] == null) {
            revs.add(rev);
          }
        });
        completer.complete(revs);
      }
    }
        
    // Work hard.
    this.db.callMethod('get', [id, optionsJs, callback]);
  ...
  Future<String> remove(var pouchable, [options]) {
    // Create jsObjects.
    JsObject pouchableJs = new JsObject.jsify(pouchable is Map ? pouchable : pouchable.toMap());
    JsObject optionsJs = new JsObject.jsify((options != null) ? options : {});
  ...
  Future<List> bulkDocs(Iterable docs, [options]) {
    var completer = new Completer();
    if (docs != null) {
      // Create jsObject.
      List list = [];
      docs.forEach((doc) => list.add(doc is Map ? doc : doc.toMap()));
      ...
      callback(JsObject error, [JsObject response]) {
        if (error != null) {
          ...
          for (int i = 0; i < resultList.length; i++) {
            ...
            pouchable['_id'] = resultElement['id'];
            pouchable['_rev'] = resultElement['rev']; 
          } 
  ...
  Object sync(String url, {toOpts: null, fromOpts: null}) {
    ...
    Object to = replicateTo(url, toOpts);
    Object from = replicateFrom(url, fromOpts);
    return {'to': to, 'from': from};
  }
  
  /// Replicate to a remote couch.
  Object replicateTo(String remote, [options]) {
    return replicate(name, remote, options);
  }
  
  /// Replicate from a remote couch.
  Object replicateFrom(String remote, [options]) {
    return replicate(remote, name, options);
  }
  ...
  static Object replicate(String source, String target, [options]) {
    if (options != null) {
      var onChange = options['onChange'];
      var onComplete = options['complete'];
    ...
    return context['PouchDB'].callMethod('replicate', [source, target, optionsJs]);
  }
  ...
  Map prepareDbChange(change) {
    ...
    var action = deleted ? DbChangeAction.DELETED : DbChangeAction.UPDATED;
    Map data = (change != null) ? change['doc'] : null;
    String id = (data != null) ? data['_id'] : null;
    String rev = (data != null) ? data['_rev'] : null;
    bool hasConflicts = (data != null && data['_conflicts'] != null && data['_conflicts'].isNotEmpty);
    return {'action': action, 'id': id, 'rev': rev, 'hasConflicts': hasConflicts, 'data': data}; 
  }
}