...
import de.freiburg.iif.model.PositionWrapper;
 ...
 */
public class PdfBoxTextStripper extends PDFStreamEngine
{

    private static final String thisClassName = PdfBoxTextStripper.class.getSimpleName().toLowerCase();
    private static float DEFAULT_INDENT_THRESHOLD = 2.0f;
    private static float DEFAULT_DROP_THRESHOLD = 2.5f;
    protected Log LOG = LogFactory.getLog(PdfBoxTextStripper.class);
     ...
     */
    // XXX Changed by CK: Changed visibility of normalize from private to protected
     ...
     */
    public PdfBoxTextStripper() throws IOException
    {
    	//XXX changed by CK*********
        super( ResourceLoader.loadProperties(
                "de/freiburg/iif/extraction/stripper/PDFTextStripper.properties", true ) );
        // *************************
     ...
     */
    public PdfBoxTextStripper( Properties props ) throws IOException
    {
        super( props );
        this.outputEncoding = null;
        normalize = new TextNormalize(this.outputEncoding);
     ...
     * @throws IOException If there is an error reading the properties.
     */
    public PdfBoxTextStripper( String encoding ) throws IOException
    {
        super( ResourceLoader.loadProperties(
    ...
    {
        try (StringWriter outputStream = new StringWriter()) {
	...
	protected void processPage( PDPage page, COSStream content ) throws IOException
    {
                ...
                (endBookmarkPageNumber == -1 || currentPageNo <= endBookmarkPageNumber ))
        {
            ...
            // XXX(CK): Added currentPageNo to endPage
            endPage( page, currentPageNo );
        }
...
//                    LOG.info("POSITION: "+position.getCharacter());
                    if(!overlap(positionY, positionHeight, maxYForLine, maxHeightForLine))
                    {
                        writeLine(normalize(line,isRtlDominant,hasRtl),isRtlDominant);
                        
                        // XXX: Added by CK *****
                        onLineExtracted(line, currentPageNo);
                        // **********************
                        line.clear();
...
//                        line.add(WordSeparator.getSeparator());
    ...
    {
//       LOG.info("   OVERLAP: y1: "+y1);
//       LOG.info("   OVERLAP: height1: "+height1);
//       LOG.info("   OVERLAP: y2: "+y2);
//       LOG.info("   OVERLAP: height2: "+height2);
//       LOG.info("   OVERLAP: within: "+within( y1, y2, .1f));
//       LOG.info("   OVERLAP: within1: "+(y2 <= y1 && y2 >= y1-height1));
//       LOG.info("   OVERLAP: within2: "+(y1 <= y2 && y1 >= y2-height2));
        return within( y1, y2, .25f) || (y2 <= y1 && y2 >= y1-height1) ||
        (y1 <= y2 && y1 >= y2-height2);
    }
     ...
     *
     */
    // XXX: Removed static by CK
    protected final class WordSeparator extends TextPosition{
      // XXX: Removed by CK.
//        private final WordSeparator separator = new WordSeparator();
     ...
     // XXX: Removed by CK.
//        public final WordSeparator getSeparator(){
//            return separator;
//        }
    ...
    // XXX: Added by CK
    protected void onLineExtracted(List<TextPosition> line, int pageNumber) {
    	// By default nothing to do, override it to define specified behaviour
    }
}